...
ZKSHELL=$(realpath $ZKSHELL)
echo "ZKSHELL $ZKSHELL"

function wait_for_zookeeper() {
    echo "wait for zookeeper"
    until $ZKSHELL --run-once "ls /" ${BK_zkServers}; do sleep 5; done
}
...
function create_zk_root() {
    if [ "x${BK_CLUSTER_ROOT_PATH}" != "x" ]; then
        ...
        $ZKSHELL --run-once "create ${BK_CLUSTER_ROOT_PATH} '' false false true" ${BK_zkServers}
    fi
}

function init_cluster() {
    $ZKSHELL --run-once "ls ${BK_zkLedgersRootPath}/available/readonly" ${BK_zkServers}
    if [ $? -eq 0 ]; then
        echo "Cluster metadata already exists"
    else
        # Create an ephemeral zk node `bkInitLock` for use as a lock.
        lock=`$ZKSHELL --run-once "create ${BK_CLUSTER_ROOT_PATH}/bkInitLock '' true false false" ${BK_zkServers}`
        if [ -z "$lock" ]; then
