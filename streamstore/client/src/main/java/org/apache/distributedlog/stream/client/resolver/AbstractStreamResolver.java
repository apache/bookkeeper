...
/**
 * The abstract zstream name resolver.
 *
 * <p>The zstream name resolvers should extend this one instead of {@link io.grpc.NameResolver}.
 */
public abstract class AbstractStreamResolver extends NameResolver {
  ...
  private final class ResolverTask implements Runnable {
    ...
    public void run() {
      ...
      synchronized (AbstractStreamResolver.this) {
        if (shutdown) {
          return;
        }
        resolving = true;
  ...
  protected AbstractStreamResolver(String name,
  ...
  public synchronized void start(Listener listener) {
    ...
    this.executor = SharedResourceHolder.get(executorService);
  ...
  public synchronized void shutdown() {
    ...
    shutdown = true;
    if (null != executor) {
      executor = SharedResourceHolder.release(executorService, executor);
    }
  }
}
