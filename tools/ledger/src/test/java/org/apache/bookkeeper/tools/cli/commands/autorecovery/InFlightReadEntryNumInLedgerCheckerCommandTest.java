...
/**
 * Unit test for {@link InFlightReadEntryNumInLedgerCheckerCommand}.
 */
public class InFlightReadEntryNumInLedgerCheckerCommandTest extends BookieCommandTestBase {

    public InFlightReadEntryNumInLedgerCheckerCommandTest() {
        super(3, 0);
    }
        ...
        mockBookKeeperAdminConstruction(
                new Consumer<BookKeeperAdmin>() {
                    ...
                    public void accept(BookKeeperAdmin bookKeeperAdmin) {
                        when(bookKeeperAdmin.getInFlightReadEntryNumInLedgerChecker()).thenReturn(1);
                        doNothing().when(bookKeeperAdmin).setInFlightReadEntryNumInLedgerChecker(anyInt());
                    }
                }
        );
    }
    ...
    public void testWithSet() throws Exception {
        ...
        verify(getMockedConstruction(BookKeeperAdmin.class).constructed().get(0),
                times(1)).setInFlightReadEntryNumInLedgerChecker(1);
    }
    ...
    public void testWithGet() throws Exception {
        testCommand("-g");
        verify(getMockedConstruction(BookKeeperAdmin.class).constructed().get(0),
                times(1)).getInFlightReadEntryNumInLedgerChecker();
    }
