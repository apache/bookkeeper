...
In order to configure BookKeeper and protect ZooKeeper just simply add a zkEnableSecurity=true configuration property on Bookie Configuration.
...
BookKeeper runtime will use ZooDefs.Ids.CREATOR_ALL_ACL ACLs for every new node.
...
- add -Djava.security.auth.login.config=conf/jaas.conf to BOOKIE_EXTRA_OPTS in conf/bkenv.sh
