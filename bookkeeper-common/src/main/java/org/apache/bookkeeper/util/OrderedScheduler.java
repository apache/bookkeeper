...
/**
 * Ordered Scheduler. It is thread pool based {@link ScheduledExecutorService}, additionally providing the ability to
 * execute/schedule tasks by <code>key</code>. Hence the tasks submitted by same <i>key</i>
 * will be executed in order.
 *
 * <p>The scheduler is comprised of multiple {@link ScheduledExecutorService}s. Each {@link ScheduledExecutorService}
 * is a single thread executor. Normal task submissions will be submitted to executors in a random manner to guarantee
 * load balancing. Keyed task submissions (e.g {@link OrderedScheduler#submit(Object, Runnable)} will be submitted to a
 * dedicated executor based on the hash value of submit <i>key</i>.
 */
