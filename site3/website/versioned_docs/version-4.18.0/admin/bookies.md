...
`journalDirectories` | The directories where the [log device](../getting-started/concepts#journals) stores the bookie's write-ahead log (WAL, as a comma-separated list) | `/tmp/bk-txn`
...
1. Ensure that all directories specified by [`journalDirectories`](../reference/config#journal-settings) and [`ledgerDirectories`](../reference/config#ledger-storage-settings) are empty.
