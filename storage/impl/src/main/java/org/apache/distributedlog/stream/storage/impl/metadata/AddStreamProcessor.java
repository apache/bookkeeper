...
/**
 * The operation process for adding stream to a storage container.
 */
class AddStreamProcessor
    extends AsyncOperationProcessor<AddStreamRequest, StorageContainerResponse, MetaRangeStoreImpl> {

  public static AddStreamProcessor of() {
    return INSTANCE;
  }

  private static final AddStreamProcessor INSTANCE = new AddStreamProcessor();

  private AddStreamProcessor() {}

  @Override
  protected StatusCode verifyRequest(MetaRangeStoreImpl state,
                                     AddStreamRequest request) {
    return state.verifyAddStreamRequest(request);
  }
  ...
  protected CompletableFuture<StorageContainerResponse> doProcessRequest(MetaRangeStoreImpl state,
                                                                         AddStreamRequest request) {
    return state.doProcessAddStreamRequest(request);
  }
}
