    ...
    public void run() {
        while (true) {
            ...
            if (hasWritableLedgerDirs || (totalDiskUsage = diskChecker.getTotalDiskUsage(ldm.getAllLedgerDirs())) < conf
                ...
                LOG.debug("Current TotalDiskUsage: {} is greater than LWMThreshold: {}. So not adding any filledDir "
                        + "to WritableDirsList", totalDiskUsage, conf.getDiskLowWaterMarkUsageThreshold());
            }
            try {
