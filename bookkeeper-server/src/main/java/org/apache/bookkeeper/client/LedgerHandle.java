...
public class LedgerHandle implements WriteHandle {
     ...
     */
    @Override
    public CompletableFuture<Long> force() {
        CompletableFuture<Long> result = new CompletableFuture<>();
        final PendingForceOp op = new PendingForceOp(this, result);
        try {
    ...
    synchronized long forceCompleted() {
        lastAddConfirmed = lastAddSyncedManager.calculateCurrentLastAddSynced();
        if (LOG.isDebugEnabled()) {
            LOG.debug("forceCompleted lastAddConfirmed {}", lastAddConfirmed);
        }
        return lastAddConfirmed;
    }

    void sendAddSuccessCallbacks() {
        LOG.info("sendAddSuccessCallbacks {}", Thread.currentThread().getName());
...
+                               pendingAddsSequenceHead);
            ...
            if (metadata.getLedgerType() == LedgerType.FORCE_DEFERRED_ON_JOURNAL) {
    ...
    void handleBookieFailure(final Map<Integer, BookieSocketAddress> failedBookies) {
        ...
        if (bk.disableEnsembleChangeFeature.isAvailable()
               || metadata.getLedgerType() == LedgerType.FORCE_DEFERRED_ON_JOURNAL) {
