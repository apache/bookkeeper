...
public class LastAddSyncedManagerTest {
    ...
    public void testCalculateLastAddSynced() throws Exception {
        assertCalculateLastAddSynced(3, 1, Arrays.asList(1L, 2L, 3L), 3);
        assertCalculateLastAddSynced(3, 2, Arrays.asList(1L, 2L, 3L), 2);
        assertCalculateLastAddSynced(3, 3, Arrays.asList(1L, 2L, 3L), 1);
    }

    @Test
    public void testCalculateLastAddSyncedNoEnoughData() throws Exception {
        assertCalculateLastAddSynced(3, 3, Arrays.asList(), -1);
        assertCalculateLastAddSynced(3, 3, Arrays.asList(1L), -1);
        assertCalculateLastAddSynced(3, 3, Arrays.asList(1L, 2L), -1);
    }

    private void assertCalculateLastAddSynced(int writeQuorumSize, int ackQuorumSize,
                                              List<Long> lastAddSynced, long expectedLastAddSynced) {
